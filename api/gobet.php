  ...
  if($balance >= $money){//余额大于下注金额,下注
    ...
    foreach ($dataarray as $key=>$value) {
        ...
        if (!$res) {
            ...
            if(!isset($remarks)){
                $sql="insert into bills(id,username,gamename,amount,surplus,type,time) values(null,'$username','$gamename',$tmpamount,$balance,5,now())";
            }else{
                $sql="insert into bills(id,username,gamename,amount,surplus,type,remarks,time) values(null,'$username','$gamename',$tmpamount,$balance,5,'$remarks',now())";
            }
    ...
    if($issuccess==false){//失败
        ...
        $mysqli->select_db($config['db']['dbname']);
        $sql="update wallet set coin=coin-$money where username='$username'";
        $res=$mysqli->query($sql);
        if($res){
            $mysqli->query("commit");
            $result['info']="押注成功";
