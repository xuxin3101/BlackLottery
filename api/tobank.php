...
if ($row) {//检查token通过
    $sql="begin;";
    $mysqli->query($sql);
    $sql="select * from wallet2 where username='$username' for update";
    $res=$mysqli->query($sql);
    $row=$res->fetch_assoc();
    $balance=$row['coin'];
    if ($balance<$amount) {
        ...
        //增加余额账单
        $surplus=$balance-$amount;
        $sql="insert into bills2(id,username,gamename,amount,surplus,type,time) values(null,'$username','钱庄转余额',-$amount,$surplus,9,now())";
        $res=$mysqli->query($sql);
        if ($res) {
            //给bills添加一个账单
            $sql="select * from bills where username='$username'";
            $res=$mysqli->query($sql);
            $row=$res->fetch_assoc();
            $bankbalance=$row['coin'];
            $bankbalance=$bankbalance+$amount;
            $sql="insert into bills(id,username,gamename,amount,surplus,type,time) values(null,'$username','钱庄转余额',$amount,$bankbalance,9,now())";
            $res=$mysqli->query($sql);
            if ($res) {//添加成功
                //更新余额
